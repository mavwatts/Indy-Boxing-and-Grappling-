
export const homeObjOne = {
    id: 'about',
    lightText: true,
    lightTextDesc: true,
    topLine: 'About',
    headline: 'Indy Boxing and Grappling',
    description: 'Indy Boxing and Grappling is a community based facility that is devoted to providing kids with top-level martial arts training and mentoring. Training sessions are currently divided into boxing, wrestling, and Brazilian Jiu Jitsu; this is not "kids MMA." Conflict resolution classes, as well as social media/internet awareness classes will be provided.',
    imgStart: true,
    img: require('../../images/svg-1.svg'),
    alt: 'about_section'
}

export const homeObjTwo = {
    id: 'signup',
    lightBg: true,
    lightText: false,
    lightTextDesc: false,
    topLine: 'Free Trial',
    headline: 'IBG offers a one week free trial on all classes.',
    description: 'Indy Boxing and Grappling is committed to transforming lives in Indianapolis by providing top-notch martial arts instruction and facilities to adolescents, teens, and select adults',
    buttonLabel: 'Learn more',
    imgStart: true,
    img: require('../../images/svg-2.svg'),
    alt: 'wrestling',
    dark: false,
    primary: false,
    darkText: true
}

export const homeObjThree = {
    id: 'location',
    lightBg: false,
    lightText: true,
    lightTextDesc: true,
    headline: 'Address and Hours',
    description: '4903 East 23rd Street Indianapolis, IN 46201',
    imgStart: false,
    img: require('../../images/svg-3.svg'),
    alt: 'Car',
    dark: true,
    primary: true,
    darkText: false
}